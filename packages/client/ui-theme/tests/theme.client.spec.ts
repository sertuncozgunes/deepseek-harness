...
describe('ThemeRuntime', () => {
  ...
  it('adopts a published Host font size without writing it back', () => {
    const { theme, events, host } = make()
    host.publish({ status: 'ready', value: { preference: 'system', fontSize: 12, accent: 'default' }, revision: 1, writable: true })
  ...
  it('adopts a published Host section without writing it back', () => {
    const { theme, events, host } = make()
    host.publish({ status: 'ready', value: { preference: 'dark', fontSize: 14, accent: 'default' }, revision: 1, writable: true })
    expect(theme.getTheme().preference).toBe('dark')
    expect(events).toHaveLength(1)
    expect(host.set).not.toHaveBeenCalled()
    host.publish({ value: { preference: 'dark', fontSize: 14, accent: 'default' }, revision: 2 })
    expect(events).toHaveLength(1)
  })
